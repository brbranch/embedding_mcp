...
### Phase 11: 追加Store実装・機能拡張

#### 15. SQLite VectorStore実装 (internal/store)
- [x] SQLite実装（modernc.org/sqlite使用、cgo不要）
- [x] cosine類似度による全件スキャン検索
- [x] embeddings を SQLite に保存（軽量用途向け）
- [x] 5,000件超過時の警告ログ出力
...
### Phase 12: 追加Embedder実装（optional）

#### 17. Ollama Embedder実装 (internal/embedder)
- [ ] Ollama embedder実装
...
**注記**: 本タスクはoptional。Embedderは抽象化されているため、必要に応じて追加実装可能。
...
- Phase 11（タスク15, 16）はStore interface完成後いつでも実施可能
- Phase 12（タスク17）はEmbedder interface完成後いつでも実施可能（optional）
